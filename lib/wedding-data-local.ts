export interface WeddingMoment {
  ...
  type: 'image' | 'video';
  localPath: string; // Usar rutas locales temporalmente
  description?: string;
  timestamp?: string;
  featured?: boolean;
}
...
// Generar datos basados en las imágenes existentes
const generateCivilMoments = (): WeddingMoment[] => {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    moments.push({
      ...
      category: 'ceremonia',
      type: 'image',
      localPath: `/galeria/civil/V&A-${num}.webp`,
...
const generateFiestaMoments = (): WeddingMoment[] => {
  ...
  fiestaNumbers.forEach((num, index) => {
    moments.push({
      ...
      category: 'fiesta',
      type: 'image',
      localPath: `/galeria/fiesta/V&A MATRIMONIO/V&A-${num}.webp`,
...
export const coupleInfo = {
  bride: {
    name: 'Valentina',
    photo: '/galeria/civil/V&A-001.webp', // Usar imagen local temporal
  },
  groom: {
    name: 'Andrés',
    photo: '/galeria/civil/V&A-002.webp', // Usar imagen local temporal
  },
